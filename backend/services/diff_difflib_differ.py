...
from schemas.diff import SectionDiff, LineDiff, InlineDiff

class DifflibDiffer:
    def diff(self, old_sections: Dict[str, List[str]], new_sections: Dict[str, List[str]]) -> List[SectionDiff]:
    ...
    def _diff_section(self, section_name: str, old_lines: List[str], new_lines: List[str]) -> SectionDiff:
        ...
        return SectionDiff(name=section_name, status=status, lines=line_diffs)
    ...
    def _diff_words(self, old_text: str, new_text: str) -> List[InlineDiff]:
        ...
        for tag, i1, i2, j1, j2 in matcher.get_opcodes():
            if tag == 'equal':
                for i in range(i2 - i1):
                    inline_diffs.append(InlineDiff(text=old_words[i1 + i], type="equal"))
            elif tag == 'replace':
                for i in range(i1, i2):
                    inline_diffs.append(InlineDiff(text=old_words[i], type="remove"))
                for j in range(j1, j2):
                    inline_diffs.append(InlineDiff(text=new_words[j], type="add"))
            elif tag == 'insert':
                for j in range(j1, j2):
                    inline_diffs.append(InlineDiff(text=new_words[j], type="add"))
            elif tag == 'delete':
